...
def SRGAN_g(t_image1, t_image2, is_train=False, reuse=False):
    ...
    with tf.variable_scope("SRGAN_g", reuse=reuse) as vs:
        ...
        n = InputLayer(t_image1, name='in')
        m = InputLayer(t_image2, name='on')
        n = Conv2d(n, 64, (3, 3), (1, 1), act=tf.nn.relu, padding='SAME', W_init=w_init, name='n64s1/c') #(3,3)卷积矩阵的高和宽，(1,1)滑动窗口尺寸的高和宽，n为(2,96,96,3)　通道数是3，因此卷积的通道也是3，这里不用写，64改通道数
        m = Conv2d(m, 64, (3, 3), (1, 1), act=tf.nn.relu, padding='SAME', W_init=w_init, name='m64s1/c')
        #temp = n
        # B residual blocks
        for i in range(16):
            ...
            n = nn
            mm = Conv2d(m, 64, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='m64s1/c1/%s' % i)
            mm = BatchNormLayer(mm, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='m64s1/b1/%s' % i)
            mm = Conv2d(mm, 64, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='m64s1/c2/%s' % i)
            mm = BatchNormLayer(mm, is_train=is_train, gamma_init=g_init, name='m64s1/b2/%s' % i)
            mm = ElementwiseLayer([m, mm], tf.add, name='mb_residual_add/%s' % i)
            m = mm
        n = Conv2d(n, 64, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n64s1/c/m')
        n = BatchNormLayer(n, is_train=is_train, gamma_init=g_init, name='n64s1/b/m')
        m = Conv2d(m, 64, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='m64s1/c/m')
        m = BatchNormLayer(m, is_train=is_train, gamma_init=g_init, name='m64s1/b/m')
        n = ElementwiseLayer([n, m], tf.add, name='add3')
...
def SRGAN_g2(t_image1, is_train=False, reuse=False):
    """ Generator in Photo-Realistic Single Image Super-Resolution Using a Generative Adversarial Network
    feature maps (n) and stride (s) feature maps (n) and stride (s)
    ...
    b_init = None  # tf.constant_initializer(value=0.0)
    g_init = tf.random_normal_initializer(1., 0.02)
    size = t_image1.get_shape().as_list()
    with tf.variable_scope("SRGAN_g", reuse=reuse) as vs:
        # tl.layers.set_name_reuse(reuse) # remove for TL 1.8.0+
        n = InputLayer(t_image1, name='in')
...
def SRGAN_d2(t_image1, is_train=False, reuse=False):
